# Governance

## Overview

This document outlines the governance structure and decision-making processes for the Motorcycle Off-Road Explorers (MORE) Club Resource Center project. As we are in our founding phase, this governance model is intentionally lightweight and designed to evolve as our community grows.

## Current Status: Founding Phase

The MORE Club is currently in its **founding phase** (as of March 2026). During this period, we are establishing our community, building our initial programs, and creating the foundation for long-term sustainability.

### Founding Phase Characteristics

- **Timeline:** March 2026 - TBD (estimated 6-12 months)
- **Primary Goal:** Build a core community of founding members
- **Governance:** Informal, consensus-based among active participants
- **Flexibility:** High - we're learning what works for our community

## Organizational Structure

### Founding Phase Structure

**Project Maintainer(s):**

- Bryan Dady ([@bcdady](https://github.com/bcdady))
- Current responsibilities: Infrastructure, website, safety content, community coordination

**Founding Members:**

- Early participants who help shape the club
- Active contributors to content, events, or community building
- Collaborative decision-making on club direction

**Contributors:**

- Anyone who submits content, code, or improvements
- Recognized for their contributions
- Input welcomed on all matters

### Future Structure (Post-Founding)

As the community matures, we plan to establish:

1. **Club Officers** (elected or volunteer-based)
   - President/Chair
   - Ride Coordinator
   - Safety Officer
   - Treasurer (if needed)

2. **Event Organizers**
   - Ride leaders
   - Workshop coordinators
   - Social event planners

3. **Content Maintainers**
   - Trail information curators
   - Safety documentation reviewers
   - Technical contributors

## Decision-Making Process

### Types of Decisions

#### 1. Technical Decisions (Website, Infrastructure)

**Who Decides:** Project maintainers and technical contributors

**Process:**

- Proposed via GitHub Issues or Pull Requests
- Discussion period (minimum 3-7 days for major changes)
- Maintainer approval required for merging
- Community input welcomed but not required

**Examples:**

- Framework updates (e.g., Docusaurus version)
- CI/CD pipeline changes
- Website architecture decisions
- Build process modifications

#### 2. Content Decisions (Safety, Trails, Resources)

**Who Decides:** Founding members and subject matter experts

**Process:**

- Proposed via GitHub Issues or Pull Requests
- Review for accuracy and safety
- Community feedback encouraged (3-5 days for major changes)
- Final approval by maintainers or designated safety reviewers

**Examples:**

- Safety guideline updates
- Trail information additions
- Riding technique tutorials
- Emergency procedure changes

#### 3. Community Decisions (Events, Policies, Direction)

**Who Decides:** Consensus among active founding members

**Process:**

- Discussed via Discord, GitHub Discussions, or email
- Minimum 7-day discussion period
- Aim for consensus; simple majority if needed
- Document decisions transparently

**Examples:**

- Event calendar and scheduling
- Membership policies
- Code of Conduct updates
- Club mission and values
- Partnership decisions

#### 4. Administrative Decisions (Day-to-Day Operations)

**Who Decides:** Project maintainers

**Process:**

- Made as needed for operational continuity
- Communicated to community when relevant
- Can be reviewed/reversed by community consensus

**Examples:**

- Email response handling
- Routine website updates
- Social media posts
- Minor content corrections

### Consensus Model

During the founding phase, we operate on a **rough consensus** model:

1. **Proposals:** Any member can propose changes or initiatives
2. **Discussion:** Open discussion period (length depends on impact)
3. **Concerns:** Serious objections must be raised and addressed
4. **Resolution:** If no serious objections remain, move forward
5. **Documentation:** Document significant decisions

**"Rough consensus"** means:

- Not everyone has to agree completely
- Serious concerns must be addressed
- The solution works for the vast majority
- Dissent is respected but doesn't create deadlock

## Roles and Responsibilities

### Project Maintainers

**Responsibilities:**

- Maintain website infrastructure and codebase
- Review and merge pull requests
- Ensure code quality and security
- Manage GitHub repository settings
- Coordinate with contributors
- Make tie-breaking decisions when needed

**Accountability:**

- Transparent decision-making
- Responsive to community input
- Follow Code of Conduct
- Act in the best interest of the community

### Founding Members

**How to Become a Founding Member:**

- Active participation during founding phase
- Meaningful contributions to club development
- Commitment to club values and Code of Conduct
- Recognition by existing founding members

**Responsibilities:**

- Participate in community decisions
- Contribute to club development (any form)
- Welcome and mentor new members
- Uphold club values and standards

**Rights:**

- Voice in major decisions
- Recognition as founding contributor
- Influence on club direction

### Contributors

**Anyone Can Contribute:**

- No formal membership required
- Follow contribution guidelines
- Abide by Code of Conduct

**Contributions Valued:**

- Code and technical improvements
- Content and documentation
- Safety expertise and reviews
- Community building and support
- Event organization and leadership

## Transparency and Communication

### Public Decision-Making

We commit to making decisions transparently:

**Public Channels:**

- GitHub Issues and Discussions for technical matters
- Discord for community discussions
- Email for sensitive matters (with summary shared when appropriate)
- Blog posts for major announcements

**Documentation:**

- Significant decisions documented in GitHub
- Meeting notes (when we have meetings) shared publicly
- Annual summaries of major changes

### Community Input

**How to Provide Input:**

1. **GitHub Issues:** For specific proposals or problems
2. **GitHub Discussions:** For broader topics and ideas
3. **Discord:** For real-time discussion and coordination
4. **Email:** more@ride-more.org for any concerns

**Response Expectations:**

- Acknowledgment within 5-7 days
- Substantive response within 2-3 weeks
- Faster for time-sensitive matters

## Conflict Resolution

### Process for Disagreements

1. **Direct Communication:** Try to resolve between involved parties
2. **Mediation:** Request mediation from uninvolved member or maintainer
3. **Community Discussion:** Bring to broader community if appropriate
4. **Maintainer Decision:** Final decision by maintainers if consensus can't be reached

### Code of Conduct Violations

See [CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md) for enforcement process.

## Evolution of Governance

### Transition from Founding Phase

When we transition from founding phase (estimated 6-12 months):

**Triggers for Transition:**

- Regular event schedule established
- Core group of 10+ active members
- Sustainable operational model
- Clear community identity

**Transition Process:**

1. Community discussion about readiness
2. Draft updated governance structure
3. 30-day review and feedback period
4. Adoption by consensus of founding members
5. Implementation of new structure

### Future Governance Models

Potential future structures we may adopt:

**Option 1: Officer-Based Structure**

- Elected or volunteer officers
- Defined terms and responsibilities
- Democratic decision-making

**Option 2: Cooperative Model**

- Member-owned and operated
- One member, one vote
- Formal membership dues and benefits

**Option 3: Non-Profit Organization**

- Formal 501(c)(7) or similar status
- Board of directors
- Bylaws and formal governance
- Tax benefits for members

**Decision:** To be made collaboratively by the community when ready

## Amendment Process

### Changing This Document

**Minor Changes** (typos, clarifications, formatting):

- Pull request by anyone
- Reviewed by maintainers
- Merged when ready

**Major Changes** (structure, processes, roles):

- Proposed via GitHub Issue or Discussion
- Minimum 14-day discussion period
- Consensus among founding members required
- Documented in pull request
- Updated "Last Modified" date

### Version History

Major changes will be tracked:

- Version number
- Date of change
- Summary of modifications
- Link to discussion/PR

## Contact and Questions

### Governance Questions

For questions about governance, decision-making, or how to get involved:

- **Email:** more@ride-more.org
- **GitHub Discussions:** [Ask a question](https://github.com/bcdady/ride-more-org/discussions)
- **Discord:** [Join our community](/discord)

### Feedback Welcome

This governance model is designed to serve our community. If something isn't working or could be improved, please speak up! We're learning together.

## Commitment to Community

Our governance is built on these principles:

1. **Transparency:** Open decision-making and communication
2. **Inclusivity:** Everyone's voice matters
3. **Safety First:** Decisions prioritize safety above all
4. **Sustainability:** Building for long-term success
5. **Adaptability:** Willing to change what doesn't work
6. **Collaboration:** Working together toward common goals

## Acknowledgment

By participating in the MORE Club community, you acknowledge that you've read and understand this governance structure and agree to participate in good faith according to these guidelines.

---

**Document Version:** 1.0  
**Last Modified:** March 2026  
**Next Review:** July 2025 or upon transition from founding phase
